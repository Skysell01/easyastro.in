...
export default function RecordPage() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const { data, error: supabaseError } = await cartSupabase
        .from("soulmate_orders")
        .select(
          "full_name, email, phone_number, project_name, date_of_birth, place_of_birth, gender, additional_products, created_at, payment_status"
        )
        .order("created_at", { ascending: false });
  ...
  useEffect(() => {
    ...
    setFilteredOrders(filtered);
  }, [orders, projectFilter, paymentFilter, dateFrom, dateTo, search]);
    ...
    <div className="flex flex-col min-h-dvh bg-background text-foreground">
      ...
      <main className="flex-1">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex flex-wrap gap-3 items-center mb-4">
            <span className="text-xs text-muted-foreground">Project:</span>
            {(["all", "English Soulmap", "Hindi Soulmap"] as ProjectFilter[]).map((p) => (
              <button
                key={p}
                onClick={() => setProjectFilter(p)}
                className={clsx(
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {(dateFrom || dateTo) && (
                <button
                  ...
                  className="px-3 py-1 rounded-full border border-border text-xs bg-muted hover:bg-muted/70 transition text-muted-foreground"
                >
                  ✕ Clear dates
                </button>
              )}
