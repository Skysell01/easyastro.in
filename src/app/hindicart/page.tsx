...
export default function CartPage() {
  ...
  /** ─────────────────────────────────────────────────────────
   *  CHECKOUT — MongoDB + Supabase dual write
   * ───────────────────────────────────────────────────────── */
  const handleCheckout = async () => {
    try {
      setIsCheckingOut(true);
      ...
      // ── 1. MongoDB: create abandoned order ──────────────────
      const abdOrderResponse = await fetch(`${BACKEND_URL}/api/lander12/create-order-abd`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const abdOrderResult = await abdOrderResponse.json();
      if (!abdOrderResult.success) throw new Error("Failed to create abandoned order");
      ...
      const result = await response.json();
      if (!result.success) throw new Error("Failed to create Razorpay order");
      ...
      const options = {
        ...
        handler: async function (response: any) {
          try {
            // ── 4a. MongoDB: create confirmed order ─────────────
            const orderResponse = await fetch(`${BACKEND_URL}/api/lander12/create-order`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
            ...
            if (orderResult.success) {
              ...
              // ── 7. MongoDB: delete abandoned order ───────────────
              await fetch(`${BACKEND_URL}/api/lander12/delete-order-abd`, {
                method: "DELETE",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ email: form.email }),
